...
const Header = function () {
  ...
  const _getUserData = async (account: string) => {
    const res = await ApiService.getUserProfile({ publicAddress: account });
    return res.data;
  };

  const _signUp = async (account: string) => {
    const res = await ApiService.createUserProfile({ publicAddress: account });
    return res.data;
  };
  ...
  const saveInLocalStorage = (ls: string | null, accessToken: string) => {
    if (!ls) {
      const tokensList = [accessToken];
      localStorage.setItem(LS_KEY, JSON.stringify(tokensList));
    } else {
      const tokensList = JSON.parse(ls);
      const updatedTokenList = [...tokensList];
      updatedTokenList.push(accessToken);
      localStorage.setItem(LS_KEY, JSON.stringify(updatedTokenList));
    }
  };
  const removeTokens = () => {
  ...
  const handleLoggedIn = async (auth: Auth) => {
    const { accessToken } = auth;
    const ls = localStorage.getItem(LS_KEY);
    saveInLocalStorage(ls, accessToken);
    const {
      payload: { _id }
  ...
  const handleLoggedOut = async () => {
    try {
      ...
      const tokensFromStorage: string[] = ls && JSON.parse(ls);

      const tokenFromStorage: string | undefined = tokensFromStorage.find(
        (accessToken) => {
          const {
            payload: { _id, publicAddress }
          } = jwtDecode<JwtDecoded>(accessToken);
          return publicAddress.toLowerCase() === userAddress;
        }
      );
      const tokensToSave = tokensFromStorage.filter(
        (token) => token !== tokenFromStorage
      );
      localStorage.setItem(LS_KEY, JSON.stringify(tokensToSave));
      ApiService.removeAuthorization();
  ...
  const loadBlockchainData = async (fromConnectButton?: boolean) => {
    try {
      ...
      if (!fromConnectButton) {
        const ls = window.localStorage.getItem(LS_KEY);
        const tokensFromStorage: string[] = ls && JSON.parse(ls);
        if (!tokensFromStorage?.length) {
          throw new Error("Token doesn't exists");
        }

        const accounts1 = accounts[0].toLowerCase();
        let userId;
        const token: string | undefined = tokensFromStorage.find(
          (accessToken) => {
            const {
              payload: { _id, publicAddress }
            } = jwtDecode<JwtDecoded>(accessToken);
            userId = _id;
            return publicAddress.toLowerCase() === accounts1;
          }
        );
        ...
        if (!userId) return; // ts problem
        const userResponse = await ApiService.getUserById({ _id: userId });
        const user = userResponse.data;
        dispatch(setUserProfile(user));
      } else {
        ...
        const accessToken = authRespond.data as string;
        ApiService.setAuthenticationToken(accessToken);
        // log in with the access token
        await handleLoggedIn({ accessToken });
      }
      ...
      if (NFT_NETWORK_DATA) {
        ...
        // const balanceEnMarketPlace = await _web3.eth.getBalance(NFT_MARKET_NETWORK_DATA.address)
        // const withdraw = await _nftMarketContract.methods.withdraw().call();
        // console.log('withdraw', withdraw);
        // console.log('balanceEnMarketPlace', balanceEnMarketPlace);
